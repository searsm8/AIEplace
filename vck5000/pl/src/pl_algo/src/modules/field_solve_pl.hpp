...
typedef void (*xform1d_t)(const float[FFT_N], float[FFT_N]);

// Apply a 1D transform to every row of a row-major NxN grid.
static void fs_rows(const float in[FFT_N * FFT_N], float out[FFT_N * FFT_N], xform1d_t f) {
row:
...
static void field_solve_pl(const float rho[FFT_N * FFT_N],
    ...
    fs_rows(rho, tmpA, dct_1d_pl);        // DCT along rows (y)
    fs_transpose(tmpA, tmpB);
    fs_rows(tmpB, tmpA, dct_1d_pl);       // DCT along rows (now x)
...
#pragma HLS PIPELINE II=1
    ...
    fs_rows(Ex, tmpA, idct_1d_pl);   fs_transpose(tmpA, Ex);      // Ex: IDCT along y, transpose
    fs_rows(Ex, tmpA, idxst_1d_pl);  fs_transpose(tmpA, Ex);      //     IDXST along x, transpose back
    fs_rows(Ey, tmpA, idxst_1d_pl);  fs_transpose(tmpA, Ey);      // Ey: IDXST along y, transpose
    fs_rows(Ey, tmpA, idct_1d_pl);   fs_transpose(tmpA, Ey);      //     IDCT along x, transpose back
}
