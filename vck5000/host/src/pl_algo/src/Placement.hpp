...
// mirrors markv1 (AIEplace.cpp), cross-checked against Xplace/DREAMPlace in the 5c audit.
//
...
// ---- markv1 initializeDensityWeight: λ = (Σ|g_wl| / Σ|g_density|) * init_multiplier ----
...
// ---- markv1 computeLipshitzEstimate: α = ||Δv|| / ||Δg_total||, clamped [1e-4, 4000] ----
...
// ---- markv1 updateGamma: γ = 10^((overflow-0.1)*20/9 - 1) * base_gamma ----
...
// ---- markv1 performNextStep momentum: a_{k+1}=(1+sqrt(4a^2+1))/2; coeff=(a_k-1)/a_{k+1} ----
...
// ---- markv1 updatePrecondWeights: w = max(1, num_pins + precond_coef*λ*(area/avg_area)) ----
...
// ---- host overflow (mirrors markv1 Grid::computeTotalOverflow) ----
