...
#include "DataBase.h"
#include "Logger.h"
#include <unistd.h>   // dup/dup2 for stdout save-restore around the C parsers
...
bool DataBase::readLEF() 
{
    ...
    {
        //disable LEF parser output by redirecting C stream stdout to /dev/null,
        // then restore the ORIGINAL stdout fd (works whether it's a tty, pipe, or file)
        fflush(stdout);
...
bool DataBase::readDEF() 
{
    ...
    //disable DEF parser output by redirecting C stream stdout to /dev/null,
    // then restore the ORIGINAL stdout fd (works whether it's a tty, pipe, or file)
    fflush(stdout);
    int saved_stdout = dup(fileno(stdout));
    freopen("/dev/null", "w", stdout);
