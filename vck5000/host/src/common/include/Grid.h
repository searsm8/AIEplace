...
struct FootprintConfig
{
    ...
    float target_density = 1.0f;  // also the movable-macro deposit weight below target_density (TODO #11b)
};

/**
 * @brief The one definition of density footprint geometry — shared by the bin scatter
 *        (Grid::computeNodeOverlaps) and the overflow metric (Placer::computeOverflow), which
...
private:
    ...
    float m_target_density = 1.0f;   // also the movable-macro deposit weight below td (TODO #11b)
