...
and marginal profiles. Usage:
    python compare_density.py MARKV1_EXACT_CSV XPLACE_NPY [--meta META.json] [--out OUT.png]
"""
...
def main():
    ...
    ap.add_argument("markv1_exact_csv")
    ...
    m = load_markv1(args.markv1_exact_csv) / td
    ...
    print(f"markv1 native {m.shape}  XPlace native {x.shape}  compared at {g}x{g}")
    print(f"{'metric':<20}{'markv1':>12}{'XPlace':>12}{'m - x':>12}")
    for k in sm:
        print(f"{k:<20}{sm[k]:>12.4f}{sx[k]:>12.4f}{sm[k]-sx[k]:>12.4f}")

    vmax = max(mg.max(), xg.max())
    fig, ax = plt.subplots(2, 3, figsize=(16, 10))
    for a, title, dat in [(ax[0, 0], "markv1 exact util", mg),
    ...
    ax[0, 2].set_title("markv1 - XPlace (blue=XPlace denser)")
    ...
    ax[1, 0].set_title(f"markv1 over-cap bins ({sm['frac_bins_over_cap']*100:.1f}%)")
    ...
    ax[1, 2].plot(mg.sum(axis=0), label="markv1", lw=2)
    ...
    fig.suptitle(f"Physical (exact) density: markv1 vs XPlace @ {g}x{g}", fontsize=14)
    fig.tight_layout()
