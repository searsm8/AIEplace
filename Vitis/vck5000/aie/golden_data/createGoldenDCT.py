...
def createGoldenFFT(filepath, size):
    ...
    print(f"Generating random input for a 2D-FFT to {filepath}")
    ...
    with open(filepath+"dct_output.dat", "w") as file, \
        open(filepath+"dct_output_tpose.dat", "w") as tpose:
        for i in range(size):
            for j in range(size):
                file.write(f"{dct_output[i][j].real:.2f}\n{dct_output[i][j].imag:.2f}\n")
                tpose.write(f"{dct_output[j][i].real:.2f}\n{dct_output[j][i].imag:.2f}\n")
...
if __name__ == "__main__":
    createGoldenFFT(golden_dir, fft_size)
