...
// Helpers lifted VERBATIM from the AIEplace pl_algo modules. This standalone test only ever
// calls dct_transpose_pass, so rather than carry dct_1d.hpp / transpose.hpp whole (their
// dct_1d/dct_row_pass/transpose_band functions are unused here), the five symbols it actually
// uses live inline. Behavior is identical; upstream homes are pl_algo's dct_1d.hpp (the three
// inline funcs) and transpose.hpp (the beat type + band height). Meow.
...
// Makhoul shuffle index: stream position i <- row index (evens increasing, odds decreasing).
