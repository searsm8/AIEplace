...
>
> This was `vck5000/pl/src/pl_algo/CHECKPOINT.md`, a running 2026-07 session log. It sat in the
> pl_algo source directory, where it read as current instructions; it is archived here (2026-08-02,
> TODO #10) because it is **history**, not state. Its value is the *why* behind today's defaults —
> keep it for that, do not follow it as a plan.
>
> **For current behavior always read the source, not this file:**
...
> - current priorities → `vck5000/0_TODO/TODO.md`
>
...
>   mirroring XPlace, which bounds the step with its backtracking line search alone (`Step.cpp`).
> - the `dct_normalize_inverse` **flag is GONE**, and its "stays opt-in" conclusion with it. The
>   double-normalized inverse was an *error*, not a frame choice; inverse transforms are now
>   unconditionally unnormalized. The 16-design MMS A/B settled it: field-faithful beat legacy
>   16/16, mean −9.6% HPWL.
> - the same retirement removed `precond_raw_area`, `precond_density_scale` and
>   `enable_pin_offsets`: the preconditioner uses **raw** node area, per-pin offsets are always on.
> - "sw_only does global placement only" is no longer true — a mixed-size PHASE 2 (macro
>   legalization + fixed-macro std-cell GP) landed 2026-07-31.
